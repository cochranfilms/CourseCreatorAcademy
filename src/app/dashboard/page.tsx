...
import { EnrollmentCertificateModal } from '@/components/EnrollmentCertificateModal';
...
export default function DashboardPage() {
  ...
  return (
    <ProtectedRoute>
      <main className="min-h-screen max-w-7xl mx-auto px-3 sm:px-4 md:px-5 py-3 sm:py-4 md:py-5 w-full overflow-x-hidden">
        ...
        {/* Profile Section */}
        <div className="bg-neutral-950/60 backdrop-blur-sm border border-neutral-800/50 p-3 sm:p-4 md:p-5 mb-4 sm:mb-5 w-full overflow-x-hidden">
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-3 md:gap-4 w-full">
            {/* Left Side - Profile Info */}
            <div className="flex-1 w-full min-w-0">
              <div className="flex items-start gap-3 mb-3">
                <div className="relative w-16 h-16 sm:w-18 sm:h-18 overflow-hidden bg-neutral-800 border-2 border-neutral-700 rounded-lg flex-shrink-0">
                  {photoURL ? (
                    <img
                      src={photoURL}
                      alt={displayName}
                ...
                <div className="flex-1 min-w-0">
                  <h1 className="text-lg sm:text-xl font-bold mb-0.5 truncate">{displayName}</h1>
                  {handle && <p className="text-neutral-400 mb-1.5 text-xs sm:text-sm truncate">@{handle}</p>}
                  {profile.title && <p className="text-white mb-1 text-xs sm:text-sm truncate">{profile.title}</p>}
                  {profile.specialties && (
                    <p className="text-red-500 mb-1.5 text-xs truncate">{profile.specialties}</p>
                  )}
                  {profile.location && (
                    <div className="flex items-center gap-1 text-neutral-300 mb-1.5 text-xs">
                  ...
                  {profile.bio && <p className="text-neutral-300 mb-2 text-xs line-clamp-2">{profile.bio}</p>}
                  {profile.skills && profile.skills.length > 0 && (
                    <div className="flex flex-wrap gap-1.5">
                      {profile.skills.slice(0, 5).map((skill, idx) => (
                        <span
                          key={idx}
                          className="px-2 py-0.5 bg-neutral-800 text-neutral-300 text-[10px] sm:text-xs border border-neutral-700 rounded"
                        >
                          {skill}
                        </span>
                      ))}
                      {profile.skills.length > 5 && (
                        <span className="px-2 py-0.5 bg-neutral-800 text-neutral-400 text-[10px] sm:text-xs border border-neutral-700 rounded">
                          +{profile.skills.length - 5}
                        </span>
                      )}
                    </div>
            ...
            <div className="flex flex-row md:flex-col gap-1.5 flex-shrink-0 w-full md:w-auto">
              <button 
                onClick={() => setShowShareModal(true)}
                className="flex items-center gap-1.5 px-2.5 sm:px-3 py-1.5 bg-neutral-900 border border-neutral-800 text-white hover:bg-neutral-800 transition text-xs whitespace-nowrap rounded"
              >
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-2.5 sm:px-3 py-1.5 bg-gradient-to-r from-orange-500 to-red-500 text-white hover:from-orange-600 hover:to-red-600 transition font-medium text-xs whitespace-nowrap rounded"
              >
                <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
                </svg>
                <span className="hidden sm:inline">{hasLegacySub ? 'Legacy+' : 'Upgrade'}</span>
              </button>
            </div>
          </div>
        </div>
          ...
          <div className="bg-gradient-to-r from-purple-950/60 to-blue-950/60 backdrop-blur-sm border border-purple-800/50 p-4 sm:p-6 mb-6 w-full overflow-x-hidden">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              >
                ...
                <div className="flex-1">
                  <div className="font-semibold text-white group-hover:text-purple-300 transition">Asset Upload</div>
                  <div className="text-sm text-neutral-400">Upload and manage assets</div>
                </div>
              ...
              >
                ...
                <div className="flex-1">
                  <div className="font-semibold text-white group-hover:text-purple-300 transition">Course Videos Upload</div>
                  <div className="text-sm text-neutral-400">Upload and manage course videos</div>
                </div>
                <svg className="w-5 h-5 text-neutral-400 group-hover:text-purple-400 transition" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
                </svg>
        ...
        <div className="flex gap-1.5 mb-3 sm:mb-4 overflow-x-auto pb-2 scrollbar-hide w-full -mx-3 sm:mx-0 px-3 sm:px-0">
          {(['projects', 'social', 'email', 'privacy', 'orders', 'onboarding', 'legacy', 'jobs', 'edit'] as const).map((tab) => (
            <button
              ...
              className={`px-2.5 sm:px-3 py-1.5 text-xs font-medium transition whitespace-nowrap flex-shrink-0 rounded ${
                activeTab === tab
        ...
        {/* Tab Content */}
        {activeTab === 'projects' && (
          <div className="bg-neutral-950/60 backdrop-blur-sm border border-neutral-800/50 p-3 sm:p-4 md:p-5 w-full overflow-x-hidden">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    onClick={addSkill}
                    className="px-4 py-2 bg-transparent border border-neutral-700 text-white hover:bg-black/20 transition"
                  >
                    Add
                  </button>
                </div>
          ...
          <div className="bg-neutral-950/60 backdrop-blur-sm border border-neutral-800/50 p-4 sm:p-6 mt-6 w-full overflow-x-hidden">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
              {listings.slice(0, 3).map((listing) => (
                <div
                  key={listing.id}
                  className="overflow-hidden border border-neutral-800 bg-transparent hover:border-neutral-700 transition-all"
                >
                  <div className="relative h-64 bg-transparent overflow-hidden">
                    ...
                    <div className={`w-full h-full bg-transparent flex items-center justify-center ${listing.images && listing.images.length > 0 ? 'hidden' : ''}`}>
          ...
          <div className="bg-neutral-950/60 backdrop-blur-sm border border-neutral-800/50 p-4 sm:p-6 mt-6 w-full overflow-x-hidden">
            ...
            <div className="space-y-3">
              {opportunities.slice(0, 3).map((opp) => (
                <div
                  key={opp.id}
                  className="border border-neutral-800 bg-transparent p-4"
                >
        ...
        {showEditModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-3 sm:p-4">
            <div className="bg-transparent border border-neutral-800 p-4 sm:p-6 max-w-2xl w-full mx-2 sm:mx-4 max-h-[95vh] sm:max-h-[90vh] overflow-y-auto rounded-lg">
              <div className="flex items-center justify-between mb-4">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    value={editDisplayName}
                    onChange={(e) => setEditDisplayName(e.target.value)}
                    className="w-full bg-transparent border border-neutral-800  px-4 py-2 text-white placeholder-neutral-500 focus:outline-none focus:ring-2 focus:ring-ccaBlue"
                  />
                </div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    placeholder="Tell us about yourself..."
                    rows={3}
                    className="w-full bg-transparent border border-neutral-800  px-4 py-2 text-white placeholder-neutral-500 focus:outline-none focus:ring-2 focus:ring-ccaBlue resize-none"
                  />
                </div>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2 mb-2">
                    {editSkills.map((skill, idx) => (
                      <span
                        key={idx}
                        className="flex items-center gap-2 px-3 py-1 bg-transparent text-neutral-300 text-sm border border-neutral-700"
                      >
                        {skill}
                        <button
                          onClick={() => removeSkill(skill)}
        ...
        {showProjectModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-3 sm:p-4">
            <div className="bg-neutral-900 border border-neutral-800 p-4 sm:p-6 max-w-3xl w-full mx-2 sm:mx-4 max-h-[95vh] sm:max-h-[90vh] overflow-y-auto rounded-lg">
              ...
              <div className="space-y-4">
                        ...
                        <button
                          ...
                          className={`px-3 py-1 text-sm border transition ${
                            projectSkills.includes(skill)
                              ...
                              : 'bg-transparent border-neutral-700 text-white hover:bg-black/20'
                          }`}
                        >
                          + {skill}
                        </button>
                ...
                <div className="flex justify-end gap-3 pt-4">
                  <button
                    ...
                    className="px-6 py-2 bg-transparent border border-neutral-800 text-neutral-300 hover:bg-black/20 transition"
                  >
                    Cancel
                  </button>
                  <button
        ...
        {user && (
          <EnrollmentCertificateModal
            ...
            userName={profile.displayName || user?.displayName || user?.email?.split('@')[0] || 'Creator'}
          />
        )}
      </main>
    </ProtectedRoute>
