...
export async function POST(req: NextRequest) {
  try {
    ...
    // Calculate the proration amount and create checkout session
    // The subscription will be updated AFTER payment succeeds in the webhook
    
    // Ensure minimum charge amount (Stripe requires at least $0.50)
    const chargeAmount = Math.max(prorationAmount, 50);
    
    // Create checkout session for the proration amount
    // After payment succeeds, we'll update the subscription in the webhook
