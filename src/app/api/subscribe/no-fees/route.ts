...
// POST /api/subscribe/no-fees
// Creates the $60/month No-Fees Membership subscription on the platform account.
// This plan allows users to skip all platform fees for marketplace sales and job listings.
export async function POST(req: NextRequest) {
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [
        {
          price_data: {
            ...
            product_data: {
              name: 'CCA No-Fees Membership',
              description: 'Skip all platform fees on marketplace sales and job listings',
            },
          },
          quantity: 1,
        },
      ...
      ...(embedded
        ...
        : {
            ...
            cancel_url: `${origin}/checkout/canceled?plan=noFees60`,
          }),
      metadata: {
        planType: 'cca_no_fees_60',
        ...(buyerId ? { buyerId: String(buyerId) } : {}),
    ...
    return NextResponse.json({ error: err.message || 'Failed to create no-fees subscription' }, { status: 500 });
  }
}
