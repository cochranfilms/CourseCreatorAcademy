...
export function CourseVideoUploadZone({ onUploadComplete, disabled }: CourseVideoUploadZoneProps) {
  ...
  const handleUpload = useCallback(async () => {
    ...
    try {
      ...
      // Step 2: Upload video using UpChunk (recommended by MUX for programmatic uploads)
      // UpChunk handles CORS, chunked uploads, and resumable uploads automatically
      // It's designed to work directly with MUX direct upload URLs
      // chunkSize is in KB - 5120 KB = 5MB = 5 * 1024 * 1024 bytes = 5,242,880 bytes
      // Must be multiple of 256KB (262,144 bytes) - 5,242,880 / 262,144 = 20 ✓
      const upload = UpChunk.createUpload({
        endpoint: uploadUrl, // Direct MUX URL - UpChunk handles CORS automatically
        file: videoFile,
        chunkSize: 5120, // 5MB chunks in KB (must be multiple of 256KB)
        method: 'PUT', // MUX direct uploads use PUT with Content-Range headers
      });
      ...
      upload.on('error', (error: any) => {
        ...
        setUploadError(error.detail?.message || error.message || 'Upload failed');
        setUploadStatus('error');
      });

      upload.on('progress', (progress: any) => {
        const percentage = progress.detail || 0;
